...
function qfl_catch_works_link($num, $post_id) {
    ...
    $link = get_permalink($works_id[$num]); 
    echo $link;                     
}
...
function qfl_catch_thumbnail_image($num, $post_id) {
    $works_id  = get_post_meta($post_id, 'works_id', false);
    $full_image_url = wp_get_attachment_image_src( get_post_thumbnail_id($works_id[$num]), 'full');
...
// global $wpdb;
// $table_name = $wpdb->prefix . "cooperation";
